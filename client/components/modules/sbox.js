...
//const search = '/api/sboxes/search';
//const profile = 'http://localhost:8080/api/profile/sboxes';
...
class SboxApp extends React.Component {
	...
	/*loadFromServer(pageSize) {
		...
		follow(client, search, [
				{rel: 'findByManagerName', params: {managerName:'user', size: pageSize}}]
		).then(sboxCollection => {
			debugger;
			return client({
				method: 'GET',
			...
			debugger;
			console.log(sboxCollection);
			console.log(sboxCollection.entity);
			debugger;
...
class SboxCreateDialog extends React.Component {
	...
	handleSubmit(e) {
		...
		this.props.attributes.forEach(attribute => {
			if (!this.refs[attribute]) input = '';
				...
				if (attribute == "anynomous") input = React.findDOMNode(this.refs[attribute]).checked;
			}
		...
		this.props.attributes.forEach(attribute => {
			if (this.refs[attribute]) React.findDOMNode(this.refs[attribute]).value = ''; // clear out the dialog's inputs
	...
	render() {
		...
		input_fields .push(<p key="description"><label htmlFor="description"> Description</label><input type="text" placeholder="description" ref="description" className="field"/></p>);
		input_fields .push(<p key="anynomous"><label htmlFor="anynomous"> Anynomous</label><input type="checkbox" placeholder="anynomous" ref="anynomous" className="field"/></p>);
		//}
...
class SboxUpdateDialog extends React.Component {
	...
	handleSubmit(e) {
		...
		var input;
		this.props.attributes.forEach(attribute => {
			if (!this.refs[attribute]) input = this.props.sbox.entity[attribute] == null? "":this.props.sbox.entity[attribute];
			else{		
				//input = React.findDOMNode(this.refs[attribute]).value.trim();
				if (attribute == "published") input = React.findDOMNode(this.refs[attribute]).checked;
			}
				console.log(attribute +': ' + input);
			updatedSbox[attribute] = input;
	...
	render() {
		...
		var input_fields = [];
		//if ("first" in this.props.links) {
		input_fields .push(<p key="published"><label htmlFor="published"> Published</label><input type="checkbox" defaultChecked={this.props.sbox.entity.published} placeholder="published" ref="published" className="field"/></p>);
		//}
